...
def extract_text_from_url(url):
    ...
    try:
        ...
        # Prioritize finding main content areas, fall back to paragraphs
...
def display_results(prediction, confidence, scores):
    """Reusable results UI block."""
    st.markdown("---")
    st.markdown("## 🎯 Origin Detection Results")
...
def main():
    ...
    if st.session_state.logged_in: 
        ...
        st.title("🤖 AI/Human Origin Detection")
        ...
        # Tabs
        tab_text, tab_url = st.tabs(["📝 Text Input", "🌐 URL Analysis"])
        ...
        with tab_text:
            ...
            # Classify button
            if st.button("🔍 Determine Origin (Text Input)", type="primary", use_container_width=True):
                if text_input and len(text_input.strip()) > 50:
                    with st.spinner("Analyzing text origin..."):
                        try:
                            ...
                            display_results(prediction, confidence, scores)
        ...
        # ----------------- URL Analysis Tab -----------------
        with tab_url:
            st.markdown("### 🌐 Analyze Article from a Web URL")
            url_input = st.text_input("Enter Article URL", placeholder="https://example.com/ai-generated-news", key="url_input_origin")
            st.caption("The article text will be extracted and analyzed for its origin.")

            if st.button("🔍 Analyze URL for Origin", type="primary", use_container_width=True, key="classify_url_button"):
                if url_input:
                    ...
                    if article_text and len(article_text.strip()) > 50:
                        st.info("✅ Article text successfully extracted. Now classifying origin...")
                        
                        with st.spinner("Analyzing text origin..."):
                            try:
                                # Predict
                                prediction, confidence, scores = classifier.predict(article_text)
                                
                                # Save to database (using URL and classification)
                                db.save_origin(
                                    st.session_state.user_id,
                                    url_input, # Save the URL instead of the long text
                                    prediction,
                                    confidence
                                )
                        ...
                        st.error("Extracted text is too short (less than 50 characters) for reliable analysis.")
                    else:
                        # Error handled in extract_text_from_url
                        pass
                else:
