        ...
        with tab1:
            ...
            with col_camera:
                ...
                if st.session_state.get("captured_images"):
                    if st.button(" Clear Captured Photos", use_container_width=True):
                        st.session_state.captured_images = []
                        st.rerun()
            ...
            if uploaded_images:
                ...
                if st.session_state.caption is None:
                    if st.button(f"Analyze {len(uploaded_images)}", key="btn_caption", use_container_width=True, type="primary"):
                    ...
                    with col1:
                        ...
                        st.markdown(f"** {len(st.session_state.all_captions)} Caption Generated**")
                    ...
                    with st.expander(f"View Detailed Analysis ({len(st.session_state.all_captions)} )"):
                    ...
                    if st.button("Generate Article", key="btn_generate", use_container_width=True, type="primary"):
                        generator = NewsGenerator()
                ...
                for i, record in enumerate(history):
                    with st.expander(f"Article #{len(history)-i} - {record['created_at'][:10]}"):
                        ...
                        with col2:
                            ...
                            if st.button(f"View Full", key=f"view_{record['id']}", use_container_width=True):
                                st.markdown("---")
